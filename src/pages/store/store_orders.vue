<template>
  <div class="find">
    ...
    <div class="top_nav">
      ...
      <div class="commodity">
        <ul>
          <li v-for="(its,index) in list_com" class="list_coms" :class="'state_'+its.order_status"
              @click="detailst(its.order_detail.order_sn)">
            <div class="com_tit">
            ...
            <div class="com_com">
              <div style="position: relative;">
                <div class="sys" v-if="its.order_detail.is_sys_order"
                     style="position: absolute;top: 0;right: 0;background: #f00;color: #fff;font-size: .12rem;">周期
                </div>
              ...
              <div class="com_com_x">
                ...
                <div class="com_com_x_ov"
                     style="width:2.6rem; overflow: hidden;text-overflow:ellipsis;white-space: nowrap;">
            ...
            <div class="director" v-if="its.server_info.appointed_row">
              ...
              <div style="margin-left: .1rem;" v-if="its.time_record.next_order_at">
                下期服务时间:{{its.time_record.next_order_at}}
              </div>
              <div style="display: flex;border-bottom: 0.01rem solid #f8f8f8; padding: 0.08rem .1rem;">
                ...
                <div class="left">
                  ...
                  <span v-for="(item, i) in its.server_info.appointed_row" style="display: flex;">
                			        	<span
                                  style="width: .3rem;height:0.3rem;line-height: .3rem; border-radius: 50%;overflow: hidden;align-items: center;flex-wrap: wrap;">
                			        		<img :src="uploadFileUrl + item.user_pic" alt="" style="width: .3rem;height:.3rem;"/>
                			        	</span>
                			     	  <span style="height:0.3rem;line-height: .3rem;margin-left: 0.05rem;"
                                    :class="{i_item : 0 == i } ">{{item.staff_name}}</span>
            ...
            its.order_detail['order_bis_state_dsc'] == 'PENDING_ORDER'" class="but_coms_but3"
                   @click.stop="sureOrder(its,index)">
                确认接单
              </div>
              <div v-if="its.order_detail['order_pay_state_dsc'] == 'PAY_SUCCESS' &&
            (its.order_detail['order_bis_state_dsc'] == 'COMPLETED' || its.order_detail['order_bis_state_dsc'] == 'CLOSED') ||
            its.order_detail['order_pay_state_dsc'] == 'REFUNDED'" class="but_coms_but1"
                   @click.stop="store_delete_order(its,index)">
                删除订单
              </div>
            ...
            its.server_info.appointed_uid.indexOf(user_id) > -1" class="but_coms_but2"
                   @click.stop="completed(its,index)">
                已完成
              </div>
            ...
            (its.order_detail['order_bis_state_dsc'] == 'SET_UP' || its.order_detail['order_bis_state_dsc'] == 'PENDING_ORDER' || its.order_detail['order_bis_state_dsc'] == 'PENDING_ASSIGN' || its.order_detail['order_bis_state_dsc'] == 'PENDING_DOOR')"
                   @click.stop="o_cancel(its,index)">
                取消订单
              </div>
              <div v-if="its.order_detail['order_pay_state_dsc'] == 'PAY_SUCCESS' && its.order_detail['order_bis_state_dsc'] == 'PENDING_EVALUATE' &&
            its.order_detail.order_rate == 1 && its.order_detail.order_comment_id != 0" class="but_coms_but2"
                   @click.stop="getComment(its.order_detail.order_comment_id)">查看评价
              </div>
            </div>
          </li>
        </ul>
          ...
          <div class="distri_biv_div2">
            <ul>
              ...
              <li v-if="item.can_assign" v-for="(item,index) in list_distri" v-else>
                <span :class="{distri_biv_div_span: distri_num.indexOf(item.id) > -1}"
                      @click.stop="tab_distri(item,index)">{{item.staff_name}}</span>
              </li>
            </ul>
          </div>
        </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        num: this.$route.query['tab_id'] || 0, //tab
        numlist: 0, //综合
        numlist2: 0, //建议方式
        inishow: false,
        ids: '', //
    ...
    methods: { //方法
      ...
      tab(index) {
        ...
        if (index == 1) {
          // 待接单
          lists.condition = {
            order_bis_state_dsc: 'PENDING_ORDER'
          }
        } else if (index == 2) {
          // 待分配
          lists.condition = {
